...
export default function MyNftMachine()
{
        ...
        if (balance.value)
        {
            ...
            balance.value.forEach((accountInfo) =>
            {
                ...
                if ((filterMint && mint == filterMint) || !filterMint)
                {
                    ...
                    metaplex.nfts().findByMint({ mintAddress: new PublicKey(mint) }).run().then(
                        (nft) =>
                        {
    ...
    async function initNftVaultMode1()
    {
        ...
        tx.add(
            program.transaction.initializeNftVault(
                poolBump,
                mintPrice,
                totalSupply,
                "",
    ...
    async function mintFromVault()
    {
        ...
        const { transaction: tx, sourceATA: buyerAta, recipientATA: vaultPoolAta } = await getAssociatedTokenAddressAndTransaction
        (
            program.provider.connection,
            nftMint,
    ...
    async function sendNfts()
    {
        ...
        for (const mint of mints)
        {
            ...
            const {transaction, sourceATA, recipientATA} =
                await getAssociatedTokenAddressAndTransaction(
                    program.provider.connection,
                    mintKey,
                    program.provider.wallet.publicKey,
                    nftVaultPool,
                ...
                <div className="flex gap-3 flex-col m-5">
                    ...
                    <div className="flex items-center justify-center">
                        ...
                        <select className="border-2 border-black p-2"
                                onChange={(e) => setNetwork(e.target.value as WalletAdapterNetwork)} value={network}>
                    ...
                    <div className="flex items-center justify-center">
                        Name:{" "}
                        <input className="border-2 border-black p-2" onChange={(e) => setName(e.target.value)} value={name}/>
                    </div>
                    <div className="flex items-center justify-center">
                        Symbol:{" "}
                        <input
                    ...
                    <div className="w-full flex justify-evenly">
                        <div className="flex flex-col justify-center items-center">
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setVaultMode1(e.target.value)}
                                    value={vaultMode1}
                                />
                            </div>
                            <div>
                                ...
                                <textarea
                                    className="w-[600px] border-2 border-black p-2"
                                    rows={5}
                                    onChange={(e) => setMints(e.target.value.split("\n"))}
                                    value={mints.join("\n")}
                                />
                            </div>

                            <div className="flex gap-2">
                                <button className="border-2 border-black p-2" onClick={fetchVault1}>Fetch Vault1</button>
                                <button className="border-2 border-black p-2" onClick={initNftVaultMode1}>Init Vault1</button>
                                <button className="border-2 border-black p-2" onClick={mintFromVault}>Mint From Vault1</button>
                                <button className="border-2 border-black p-2" onClick={sendNfts}>Send NFTs to Vault1</button>
                            </div>

                            <div className="p-2">
                                Mode 1 Description:
                                <ul>
                                    <li>Create a new vault by clicking the Init Vault Mode 1 with entering the total
                                        supply and mint price
                                    </li>
                                    <li>Use already created vault by entering the public key</li>
                                    <li>After initialize or enter the public key, have to send NFTs to the vault mod 1
                                        by entering the mint addresses in the textarea and clicking the send buton
                                    </li>
                        ...
                        <div className="flex flex-col justify-center items-center">
                            <div>
                                ...
                                <input
                                    className="w-[600px] border-2 border-black p-2"
                                    onChange={(e) => setVaultMode2(e.target.value)}
                                    value={vaultMode2}
                                />
                            </div>
                            <div>
                                ...
                                <textarea
                                    className="w-[600px] border-2 border-black p-2"
                                    rows={5}
                                    onChange={(e) => setHashes(e.target.value.split("\n"))}
                                    value={hashes.join("\n")}
                                />
                            </div>
                            <div className="flex gap-2">
                                <button className="border-2 border-black p-2" onClick={fetchVault2}>Fetch Vault2</button>
                                <button className="border-2 border-black p-2" onClick={initNftVaultMode2}>Init Vault2</button>
                                <button className="border-2 border-black p-2" onClick={addUris}>Add Uris to Vault2</button>
                                <button className="border-2 border-black p-2" onClick={mint}>Mint Mode 2</button>
                            </div>
                            <div className="p-2">
                                Mode 2 Description:
                                <ul>
                                    <li>Create a new vault by clicking the Init Vault Mode 2 with entering the total
                                        supply and mint price, name, symbol, creator
                                    </li>
                                    <li>Use already created vault by entering the public key</li>
                                    <li>After initialize or enter the public key, have to send uri hashes to the vault
                                        mod 2 by entering the hashes list in the textarea and clicking the add uris
                                        button
                                    </li>
                                    <li>Then can mint from vault by clicking the mint button</li>
                                </ul>
