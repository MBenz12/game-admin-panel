...
export const programId = new PublicKey(idl_coinflip.metadata.address);
...
const programID = idl_coinflip.metadata.address;
...
async function getAddPlayerTransaction(program: Program<Coinflip>, provider: Provider, game_name: string, game_owner: PublicKey) {
...
export async function playTransaction(program: Program<Coinflip>, provider: Provider, wallet: WalletContextState, game_name: string, game_owner: PublicKey, betNo: number) {
  const [game] = await getGameAddress(game_name, game_owner);
...
export async function withdrawTransaction(program: Program<Coinflip>, provider: Provider, wallet: WalletContextState, game_name: string, game_owner: PublicKey)
{
    const [game] = await getGameAddress(game_name, game_owner);
    const [player] = await getPlayerAddress(provider.wallet.publicKey, game);
