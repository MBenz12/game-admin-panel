...
import { Cluster, clusterApiUrl, Connection, LAMPORTS_PER_SOL, ParsedInstruction, ParsedTransactionWithMeta, PublicKey, Signer, SYSVAR_RENT_PUBKEY, Transaction } from "@solana/web3.js";
...
import {
  ...
  getBuyTicketTransactionBySPL,
  getInitRaffleTransaction,
  getRaffleAccount,
  getRaffleFinalizeTransaction,
  getTransferSPLTokenTransaction
...
export default function Rust() {
  ...
  async function updateMetaDataToken() {
    ...
    const metax = new CreateMetadataV2(
  ...
  async function convertKCtoSKTFinalized() {
    ...
    for (const user of users) {
      //.slice(0, sliceMax))
      if (user.txId && !user.txIdConfirmed) {
        // && (user.userId == 216834139989606401 || user.userId == 697870826640375819 || user.userId == 772732866844688385))
        try {
          // @ts-ignore
  ...
  async function convertKCtoSKT_Start() {
    ...
    for (const user of users) {
      ...
      //continue;
      if (transactionCount >= maxPerTransactions) break;

      const userId = user.userId;
      const userName = user.userName;
      const userCoins = user.userCoins;
      const userWallets: [] = user.walletAddress;
      const userWalletsMap = userWallets.map((walletItem: any) => {
        return walletItem.walletAddress;
      });
      let userWallet = userWalletsMap[0];

      if (user.userName === "JokerKitty") userWallet = userWalletsMap[3];
  ...
  //     //   const { nft: updatedNft }: any = await metaplex
  //     //     .use(keypairIdentity(keypair))
  //     //     .nfts()
  //     //     .update({
  //     //       nftOrSft: nftRes,
  ...
  function getMetaplex(): Metaplex {
    ...
    const metaplex = Metaplex.make(connection).use(walletAdapterIdentity(wallet)).use(bundlrStorage(bundlrNetwork));
  ...
  async function mintTokenFromMainnet() {
    ...
    const nftBuilder = await metaplexMainnet.nfts().create(onchain_json).run();
  ...
  async function mintFromOnChainJSON(json: any, mintAmount: number) {
    ...
    for (let i = 1; i <= mintAmount; i++) {
      const nftBuilder = await metaplex.nfts().builders().create(json);
    ...
    await Promise.all(
      signedTXs.map(async (signedTx: Transaction, index: number, array: Transaction[]) => {
  ...
  async function fetchGlobal() {
    ...
    const out = {
      ...
      authorizedAdmins: globalAccount!.authorizedAdmins.map((authAdminPubKey) => {
        return authAdminPubKey.toString();
      }),
    };
  ...
  async function withdrawFromPda() {
    ...
    const [rafflePool] = await PublicKey.findProgramAddress([Buffer.from("raffle_pool"), raffle.toBuffer()], program.programId);
  ...
  async function buyTicketSOL() {
    ...
    // const blockheightBasedTransactionConfirmationStrategy: BlockheightBasedTransactionConfirmationStrategy = {
  ...
  async function withdrawVault() {
    ...
    const [_vaultPool] = await PublicKey.findProgramAddress([Buffer.from(VAULT_SKT_SEED_PREFIX), vaultAddressPubKey.toBuffer()], program.programId);
    ...
    const tran = await program.transaction.withdrawVault(new anchor.BN(withdrawSPLAmount * LAMPORTS_PER_SOL), new anchor.BN(withdrawSOLAmount * LAMPORTS_PER_SOL), {
      accounts: {
        global,
  ...
  async function approve() {
    ...
    const nftAta = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, nftMint, provider.wallet.publicKey, false);
    const transaction = new Transaction();
    transaction.add(Token.createApproveInstruction(TOKEN_PROGRAM_ID, nftAta, delegator, provider.wallet.publicKey, [], 1));
    // transaction.add(
  ...
  async function revoke() {
    ...
    const nftAta = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, nftMint, provider.wallet.publicKey, false);
    const transaction = new Transaction();
    transaction.add(Token.createRevokeInstruction(TOKEN_PROGRAM_ID, nftAta, provider.wallet.publicKey, []));
    // transaction.add(
    //     Token.createApproveInstruction(
    //         NATIVE_MINT,
    //         provider.wallet.publicKey,
  ...
  async function transfer() {
    ...
    transaction.add(Token.createTransferCheckedInstruction(TOKEN_PROGRAM_ID, sourceATA, nftMint, recipientATA, owner, [], 1, 0));
    const tx = await wallet.sendTransaction(transaction, connection);
    console.log(`Revoke successful, tx: ${tx}`);
  ...
  async function initVault() {
    ...
    let transaction = await program.transaction.initializeVault(bump, SystemProgram.programId, {
      accounts: {
        payer: provider.wallet.publicKey,
    ...
    const sourceATA = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, tokenSPLAddress, sktTempWallet.publicKey);

    const recipientATA = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, tokenSPLAddress, _vaultPool, true);

    const instruction = Token.createTransferCheckedInstruction(TOKEN_PROGRAM_ID, sourceATA, tokenSPLAddress, recipientATA, sktTempWallet.publicKey, [], transferToVaultAmount * LAMPORTS_PER_SOL, 9);
      ...
      <div className="flex flex-col items-center gap-2 relative">
            ...
            )}
            {
              <div>
                <input
                  ...
                  style={{ width: 400 }}
                  defaultValue={"Enter Raffle Address..."}
                />
              </div>
            }
            {
              <div>
                <input
                  ...
                  onFocus={(t) => t.target.select()}
                  style={{ width: 400 }}
                  defaultValue={"Enter Nft Winner Tx..."}
                />
              </div>
            }
            {
...

        {
          <h3 className="text-[20px] font-bold">
            ...
            <input
              ...
              onChange={(e: any) => {
                setRaffleAddress(new PublicKey(e.target.value));
                console.log("Raffle Address Changed:", raffleAddressInput.current!.value);
              }}
              defaultValue={"Insert Raffle Address..."}
              style={{ width: 400 }}
            />
...

        {
          <h3 className="text-[20px] font-bold">
            ...
            <br />
            <div>
              <pre style={{ textAlign: "left", display: "flex", justifyContent: "space-around" }}>{JSON.stringify(raffleOutput, null, "\t")}</pre>
            </div>
          </h3>
        }
